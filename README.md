...
- `PokeApiPokemonFetcher` ainda é um service grande (1000+ linhas) — candidato a decomposição, hoje adiada por falta de testes de regressão cobrindo sua lógica.
- Cobertura de testes ainda é inicial (unitários + smoke tests); services de domínio carecem de testes de integração.
- `phpstan-baseline.neon` documenta débito técnico pré-existente aceito conscientemente (não escondido, apenas não resolvido ainda).
