...
|---|---|
| `Controller/AdminController` | Entrada do painel (`/admin`) |
...
Nas rotas públicas, cada domínio tem seu próprio controller: `PokemonController` (detalhe, busca de golpes), `TeamController` (montador de times), `TierListController`, `TrainerCardController` (perfil, seguidores), `MovesetController`, `DiscordApiController` (API para bots), `SecurityController` (login/registro/OAuth Google).
