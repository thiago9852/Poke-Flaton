(() => {
	function initMoveSearch() {
		...
		if (moveInput && autocompleteResults) {
			if (!moveInput.dataset.searchInitialized) {
				moveInput.dataset.searchInitialized = 'true';
						...
						try {
							...
							if (Array.isArray(data) && data.length > 0) {
								data.forEach((move, idx) => {
									...
									/* const tmBadgeHtml = move.tm_code 
										? `<span style="margin-left: 8px; font-size: 0.75rem; background: rgba(139, 92, 246, 0.2); color: #a78bfa; padding: 2px 6px; border-radius: 4px; border: 1px solid rgba(139, 92, 246, 0.4);">${move.tm_code}</span>` 
										: ''; */
									const baseCountHtml = (move.base_pokemon_count !== undefined && move.base_pokemon_count > 0)
										? `<span style="font-size: 0.75rem; color: var(--text-muted); margin-left: 8px;">(${move.base_pokemon_count} Base)</span>`
										: '';

									a.innerHTML = `
										<div class="autocomplete-item-info" style="text-align: center; width: 100%; display: flex; align-items: center; justify-content: center; gap: 4px;">
			...
			if (window.moveSearchClickHandler) {
				document.removeEventListener('click', window.moveSearchClickHandler);
			}
			window.moveSearchClickHandler = function (e) {
				if (!moveInput.contains(e.target) && !autocompleteResults.contains(e.target)) {
					autocompleteResults.style.display = 'none';
				}
			};
			document.addEventListener('click', window.moveSearchClickHandler);
		}
	}

	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', initMoveSearch);
	} else {
		initMoveSearch();
	}
	document.addEventListener('turbo:load', initMoveSearch);
})();
